...
(function() {
  ...
  if (!btnWrite || !svg || !logContent) return;
  ...
  async function handleWrite() {
    ...
    if (state.activePrimary === 'primary') {
      ...
      animatePacket('path-wal-s1', '#ff4d4d', 800).then(function() {
      ...
      if (state.standby2State !== 'OFFLINE' && state.standby2State !== 'PROVISIONING') {
        animatePacket('path-wal-s2', '#ff4d4d', 800).then(function() {
  ...
  async function handleFailover() {
    ...
    setTimeout(function() { playBeep(180, 'triangle', 0.8); }, 200);
